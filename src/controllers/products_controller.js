...
const createProduct = async (req, res) => {
  ...
  db_connection.getConnection((err, conn) => {
    if (err) {
      ...
      queryAsync(findSkuQuery)
        .then((skuResult) => {
          if (skuResult.length > 0 && skuResult[0].sku != "") {
            return res
              .status(400)
              .json({ message: `the sku already exists in the database` });
          } else {
            ...
            queryAsync(insertProductQuery)
              .then((rs) => {
                const findLastProductQuery =
                  "select product_id from products order by product_id desc limit 1";
                queryAsync(findLastProductQuery)
                  .then((productId) => {
...
const updateProduct = async (req, res) => {
  ...
  db_connection.getConnection((err, conn) => {
    ...
    conn.beginTransaction();
    queryAsync(updateProductQuery, values)
      .then(() => {
        if (updates.isVariable === 1) {
          if (sizes && sizes.length > 0) {
            for (const s of sizes) {
              if (s.size_id) {
                const sizeValues = [s.size, s.sku, s.stock, s.size_id];
                // console.log('u',sizeValues)
                const updateSizeQuery = `UPDATE sizes SET size=?, sku=?, stock=? WHERE size_id=?`;
                queryAsync(updateSizeQuery, sizeValues)
                  .then()
                  .catch((e) => {
                    conn.rollback();
                    return res.status(500).json("Server Error: " + e);
  ...
  db_connection.query(query, (error, result) => {
    if (error) {
      ...
      return res
        .status(200)
        .json({
          message: `stock of product with id: ${productId} updated successfully`,
        });
    }
  });
};
  ...
  if (IsANumber(returnId)) {
    ...
    db_connection.query(query, (error, result) => {
      if (error) {
        ...
        return res
          .status(200)
          .json({ message: `Return canceled successfully` });
      }
    });
  } else {
    return res
      .status(404)
      .json({ ErrorMessage: `the param ${returnId} must be an integer` });
  }
};
...
const createReturn = (req, res) => {
  ...
  if (IsANumber(userId) && IsANumber(customerId)) {
    ...
    return res
      .status(404)
      .json({
        ErrorMessage: `the param ${userId} or ${customerId} must be an integer`,
      });
  }
};
  ...
  if (IsANumber(productId) && IsANumber(returnId)) {
    ...
    db_connection.query(query, (error, result) => {
      if (error) {
        ...
        return res
          .status(200)
          .json({ message: `Return details created successfully` });
      }
    });
  } else {
    return res
      .status(404)
      .json({
        ErrorMessage: `the productId param ${productId} or ${returnId} must be an integer`,
      });
  }
};
//--------------------------------------------------*** END Returns Section *---------------------------------------/
...
const createSizeByProductId = (req, res) => {
  ...
  if (IsANumber(productId)) {
        ...
        if (result.length > 0 && result[0].sku != "") {
          ...
          db_connection.query(query, values, (error, result) => {
            if (error) {
              ...
              const findLastSizeIdQuery =
                "SELECT size_id FROM sizes order by size_id DESC LIMIT 1";
              db_connection.query(findLastSizeIdQuery, (error, result) => {
                if (error) {
                  return res
                    .status(500)
                    .json("Server Error finding last id: " + error);
                } else {
                  return res
                    .status(200)
                    .json({
                      lastSizeIdCreated: result[0].size_id,
                      message: `Size created to product with id: ${productId} successfully`,
                    });
                }
              });
            }
    ...
    return res
      ...
      .json({
        ErrorMessage:
          "the productId param " + productId + " must be an integer",
      });
  }
};
  ...
  if (IsANumber(sizeId)) {
    ...
    db_connection.query(query, (error, result) => {
      if (error) {
        ...
        return res
          .status(200)
          .json({ message: `Size with id: ${sizeId} deleted successfully` });
      }
    });
  } else {
    return res
      .status(404)
      .json({
        ErrorMessage: "the sizeId param " + sizeId + " must be an integer",
      });
  }
};
...
const createProductInventory = async (req, res) => {
  ...
  db_connection.getConnection((err, conn) => {
    ...
    conn.beginTransaction();
    queryAsync(updateGeneralStockQuery)
      .then(() => {
        if (product.is_variable === 1 && sizes.length > 0) {
          for (const size of sizes) {
            if (size.qty > 0) {
              const updateSizeStockQuery = `UPDATE sizes SET stock = stock ${sign} ${parseFloat(
                size.qty
              )} WHERE size_id = ${size.size_id}`;
              queryAsync(updateSizeStockQuery)
                .then(() => {
                  const isertInventoryQuery = `INSERT INTO inventory (operation_type,quantity,reason,product_id,size_id,user_id) values ('${operationType}',${parseFloat(
                    size.qty
                  )},'${reason}',${product.product_id},${
                    size.size_id
                  },${userId})`;
                  queryAsync(isertInventoryQuery)
                    .then()
                    .catch((e) => {
                      conn.rollback();
                      return res.status(500).json("Server Error: " + e);
          ...
          const isertInventoryQuery = `INSERT INTO inventory (operation_type, quantity, reason, product_id, size_id, user_id) values ('${operationType}', ${quantity}, '${reason}', ${
            product.product_id
          }, ${null}, ${userId})`;
          queryAsync(isertInventoryQuery)
            .then()
